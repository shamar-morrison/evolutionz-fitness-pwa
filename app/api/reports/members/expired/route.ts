...
import { readMemberExpiredReport } from '@/lib/member-reports-server'
...
export async function GET(request: Request) {
  try {
    ...
    const report = await readMemberExpiredReport(supabase, filters)
    ...
    return createMemberReportErrorResponse(
      error instanceof Error
        ? error.message
        : 'Unexpected server error while loading the member expiry report.',
      500,
    )
  }
}
