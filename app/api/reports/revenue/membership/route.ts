...
import { readMembershipRevenueReport } from '@/lib/revenue-reports-server'
...
export async function GET(request: Request) {
  try {
    ...
    const report = await readMembershipRevenueReport(supabase, filters)
    ...
    return createUnexpectedRevenueReportErrorResponse(
      error,
      'Unexpected server error while loading the membership revenue report.',
    )
  }
}
