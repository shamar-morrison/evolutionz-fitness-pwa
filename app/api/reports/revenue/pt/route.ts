...
import { readPtRevenueReport } from '@/lib/revenue-reports-server'
...
export async function GET(request: Request) {
  try {
    ...
    const report = await readPtRevenueReport(supabase, filters)
    ...
    return createUnexpectedRevenueReportErrorResponse(
      error,
      'Unexpected server error while loading the PT revenue report.',
    )
  }
}
