...
import type { Member, MemberGender, MemberRecord, MemberType } from '@/types'
...
export type ProvisionMemberAccessInput = {
  name: string
  type: MemberType
...
export type ProvisioningAdminClient = AccessControlJobsClient & {
  from(table: 'members'): {
...
async function buildDeviceRollbackResult(
  baseError: string,
  employeeNo: string,
  supabase: AccessControlJobsClient,
  status: number,
  ...
  return {
    ok: false,
    error: buildRollbackError(baseError, rollbackResult, rollbackError),
    status,
  }
}

export async function provisionMemberAccess(
  input: ProvisionMemberAccessInput,
  ...
  if (addUserJob.status !== 'done') {
    return {
      ok: false,
      error: buildAddUserFailureMessage(addUserJob.error),
      status: addUserJob.httpStatus,
    }
  }
  ...
  try {
    ...
    if (getCardJob.status !== 'done') {
      return await buildDeviceRollbackResult(
  ...
  if (existingCardEmployeeNo && existingCardEmployeeNo !== employeeNo) {
    try {
      const revokeCardJob = await revokeAssignedCard(existingCardEmployeeNo, normalizedCardNo, supabase)
  ...
  try {
    ...
    return {
      ok: false,
      error: buildPersistenceRollbackError(
